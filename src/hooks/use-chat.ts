...
let setPanelContentGlobal: (content: { title: string; content: React.ReactNode }) => void;
let setShowPanelGlobal: (show: boolean) => void;
let setMessagesGlobal: React.Dispatch<React.SetStateAction<Message[]>>;

export const handleShowDiagram = (diagramUrl: string) => {
  setPanelContentGlobal({
    title: "Diagram View",
    content: <img src={diagramUrl} alt="Diagram" className="max-w-full h-auto rounded-lg" />
  });
  setShowPanelGlobal(true);
};

export const handleOptionSelect = (messageId: string, optionIndex: number) => {
  setMessagesGlobal(prevMessages => 
        ...
        if (message.selectedOption === undefined || message.selectedOption !== optionIndex) {
          ...
          if (isCorrect && !message.answeredCorrectly) {
            setTimeout(() => {
              ...
              setMessagesGlobal(prev => [...prev, badgeMessage]);
            }, 1000);
          }
        }
...
export const useChat = () => {
  ...
  // Set global references for functions that need to be used outside the hook
  setMessagesGlobal = setMessages;
  setPanelContentGlobal = setPanelContent;
  setShowPanelGlobal = setShowPanel;
